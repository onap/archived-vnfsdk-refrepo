...
public class VTPScenarioResource extends VTPResource{
    ...
    public VTPTestCase getTestcaseHandler(String scenario, String testSuiteName, String testCaseName) throws VTPException {
        ...
        try {
            results = this.makeRpcAndGetJson(args).getAsJsonObject();
        } catch (IOException e) {
            LOG.error("IOException occurs",e);
        }

        JsonObject schema = results.getAsJsonObject("schema");
        ...
        if (inputsJson != null && inputsJson.isJsonArray()) {
            for (final JsonElement jsonElement: inputsJson) {
        ...
        if (outputsJson != null && outputsJson.isJsonArray()) {
            for (final JsonElement jsonElement: outputsJson) {
