        ...
        if (results != null && results.isJsonArray()) {
            ...
            if (resultsArray.size() >= 0) {
                for (Iterator<JsonElement> it = resultsArray.iterator(); it.hasNext();) {
                    JsonElement n = it.next();
                    if (n.getAsJsonObject().entrySet().iterator().hasNext()) {
                        list.getSuites().add(new VTPTestSuite().setName(n.getAsJsonObject().get("service").getAsString()).setDescription(
                                n.getAsJsonObject().get(DESCRIPTION).getAsString()));
                    }
                }
            }
                    ...
                    if (n.getAsJsonObject().entrySet().iterator().hasNext())
                        list.getTestCases().add(
                                new VTPTestCase().setTestCaseName(
                                        n.getAsJsonObject().get("command").getAsString()).setTestSuiteName(
                                                n.getAsJsonObject().get("service").getAsString()));
                }
            }
        }
